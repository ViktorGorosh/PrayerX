...
import {getColumnsService, postColumnService, updateColumnService} from "../../services/columnService";
import {getColumnsSuccess, postColumnSuccess, updateColumnSuccess} from '../ducks/column'
...
function* postColumn(action: PayloadAction<ColumnForPost>) {
  ...
  try {
    ...
    if (data.title && data.id && data.user) { // Server must return new column
...
function* updateColumn(action: PayloadAction<ColumnUpdate>) {
  yield put(loadingOn())
  try {
    const data: UpdateColumnResponseData = yield call(updateColumnService, action.payload);
    if (data.title && data.id && data.userId) { // Server must return updated column
